...
public class T04ServletCookieTest extends HttpServlet {
	 ...
	 * 쿠키의 도메인이 쿠키를 생성한 서버의 도메인을 벗어나면 브라우저는 쿠키를 저장(생성)하지 않는다. (보안위험 때문) - 경로 : 쿠키를
	 * 공유할 기준경로를 지정한다.(도메인 이후 부분으로 디렉토리 수준) => 지정하지 않으면 URL경로 부분이 사용된다.
	 * 
	 * 2. 동작 방식 - 쿠키 생성단계 : 생성한 쿠키를 응답메세지의 헤더에 저장하여 웹브라우저에 전송한다. - 쿠키 저장단계 : 웹브라우저는
	 * 응답데이터에 포함된 쿠키를 쿠키저장소에 보관한다. - 쿠키 전송단계 : 웹브라우저는 저장한 쿠키를 요청이 있을 때마다 웹서버에
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 쿠리 정보 삭제 예제
		delteCookieExam(req, resp);
	}

	private void delteCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		/*
		 * 사용중인 쿠키정보를 삭제...
		 * 
		 * 1. 사용중인 쿠키정보를 이용하여 쿠키객체를 생성
		 * 
		 * 2. 쿠키객체의 최대지속시간을 0으로 설정
		 * 
		...
		out.print("<!DOCTYPE html><html><title>" + title + "</title></head>" + "<body>");
	...
	private void readCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		// 현재 도메인에서 사용중인 Cookie객체 배열 가져오기
		Cookie[] cookies = req.getCookies();
	...
	private void setCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		// 쿠키 소멸시간 설정하기(초단위) => 지정하지 않으면 웹브라우저를 종룔할 떄 쿠키를 함께 삭제한다.
		userId.setMaxAge(60 * 60 * 24); // 1일
		// Jaavascript를 이용한 직접 접근 방지(XSS 공격대비)
		// document.cookie를 입력하여도 userid는 접근 불가
		...
		out.print("<!DOCTYPE html><html><head><title>" + title + "</title></head>" + "<body>" + "<h1 align=\"center\">"
				+ title + "</h1>" + "<ul>" + "<li><b> ID : </b>" + req.getParameter("userId") + "</li>"
